Scriptname ED_BloodTentacleTrapHit_Script extends ObjectReference  
;
;
;
;=========================================
...
; 		;debug.Trace(self + " has hit " + akTarget)
		if !(akTarget as actor).hasSpell(ghostAbility) && !(akTarget as actor).IsInFaction(noHitFaction)
			...
			akTarget.ProcessTrapHit(self, damage, trapPushBack, 0.0, 0.0, 1000.0, 0.0, 0.0, 0.0, aeMaterial, staggerAmount)
			debug.Trace("Everdamned DEBUG: Blood Tentacle Hit stats; Damage: " + damage + \
						", trapPushBack: " + trapPushBack + \
						", afXVel: " + afXVel + ", afYVel: " + afYVel + ", afZVel: " + afZVel + \
						", afXPos: " + afXPos + ", afYPos: " + afYPos + ", afZPos: " + afZPos + \
						", aeMaterial: " +  aeMaterial + ", staggerAmount: " + staggerAmount)
			if throttleSound
				if soundCanPlay
					soundCanPlay = False
					if TrapHitSound
